...
void irq_handler (struct trapframe *r)
{
    ...
    // Force a reschedule when returning from an interrupt (mainly the timer
    // tick) so the scheduler is preemptive: a CPU-bound process is taken off
    // the CPU each tick and a fresh lottery is held among the RUNNABLE
    // processes. Without this, a process that never sleeps would run forever.
    // Only yield if we interrupted a running process (proc != NULL and
    // RUNNING) -- not the scheduler's idle loop. An IRQ can only fire while
    // ncli == 0 (holding a spinlock masks interrupts), so it is safe to take
    // ptable.lock and call sched() here.
