...
void micro_delay (int us)
{
    ...
    // Note: This integer math works only if CLK_HZ >= 1,000,000.
    uint ticks_needed = (CLK_HZ / 1000000) * us;

    // Handle cases where the clock is too slow for microsecond precision
    if (ticks_needed == 0) {
        ticks_needed = 1; // At least wait for one tick
    }

    timer1[TIMER_CONTROL] = TIMER_EN | TIMER_32BIT;
    timer1[TIMER_LOAD] = ticks_needed;
