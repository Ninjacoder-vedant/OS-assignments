...
int fork(void)
{
    ...
    *np->tf = *proc->tf;
    ...
    np->boostleft = 0; // do not inherit from the process
    np->sleepUntil = 0;
...
struct proc *hold_lottery(uint total_tickets)
{
    if (total_tickets <= 0)
    {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        ...
        {
            p->boostleft--;
            curr_tickets += tickets;
        }
        curr_tickets += tickets;
        if (curr_tickets>winner_ticket_number)
            return p;
        // pick the winning process from ptable.
        // return winner.
    }
    cprintf("This function should only be called when at least 0 process is RUNNABLE");
    return 0;
}
...
void scheduler(void)
{
    ...
    for (;;)
    {
        ...
        winner_p = hold_lottery(totalTickets);
...
int get_procs()
{
    struct pstate *pstate_user;
    struct proc *p;
    int i = 0;
    if (argptr(0, (void *)&pstate_user, sizeof(struct pstate) * NPROC) < 0)
        return -1;
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    {
        if (p->state != UNUSED)
        {
            pstate_user[i].inuse = 1;
            pstate_user[i].pid = p->pid;
            pstate_user[i].ppid = p->parent ? p->parent->pid : 0;
            safestrcpy(pstate_user[i].name, p->name, sizeof(p->name));
            safestrcpy(pstate_user[i].state, states[p->state], sizeof(pstate_user[i].state));
            pstate_user[i].syscalls = p->syscall_count;
            i++;
        }
    }
...
int getpinfo(struct pstat *uptr)
{
    struct pstat local;
    struct proc *p;
    int i = 0;
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++, i++) {
        local.inuse[i] = (p->state != UNUSED);
        local.pid[i] = p->pid;
        local.tickets[i] = p->tickets;
        local.runticks[i] = p->runticks;
        local.boostsleft[i] = p->boostleft;
    }
    release(&ptable.lock);

    // copy kernel buffer → user space
    if (copyout(proc->pgdir, (uint)uptr, (char*)&local, sizeof(local)) < 0)
        return -1;
