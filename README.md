...
This repo contains the assignments that build useful features on top of the stock xv6-pi5 operating system. Each `assignment-x` folder contains an `OS` folder that follows the structure described in the upcoming section, along with a README that explains which feature it implements on top of this base. The tools required are the same for every assignment, and are described in the following section.
...
1. **Build the Kernel and User Programs**:
